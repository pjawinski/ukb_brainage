...
cat ${targetDir}/input.txt | awk '{ print $1, $7}' OFS='\t' > "${targetDir}/sumstats4clumping.txt"
...
awk -F'\t' -v header="${header}" '
	...
	if [ ! -z "$missings" ]; then
		...
		awk -F'\t' -v header="$header" 'BEGIN { print header }
			...
			FNR==1 { next } 
			$(NF-5) in id { for(i=1;i<=NF;++i) { length(output)==0 ? output = $i : output = output"\t"$i }; output = output"\t"id[$(NF-5)]; print output; output=""; next }
...
awk -F'\t' -v header="${header}" 'BEGIN { print header } NR==FNR { id[$3":"$4":"$11":"$12]=$0; next}
	...
	OFS='\t' "${targetDir}/conditional.cleaned.tophits.annovar.txt" <(cat ${targetDir}/annovar/multianno/conditional.cleaned.tophits.annovar.hg19_multianno.txt | grep 'nonsynonymous\|frameshift\|stopgain\|stoploss') \
