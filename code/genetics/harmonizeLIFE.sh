...
targetDir="$3" # targetDir="results/gap_gm/replicate/LIFE"
...
(for chr in {1..22} X; do (
	...
	# match variant ids
	if [ ${chr} == "1" ]; then echo "[1/8] Matching variants by chr_bp_ref_alt or chr_bp_alt_ref."; fi
	...
	if [ ${chr} == "1" ]; then echo "[2/8] Removing potential duplicates."; fi
	...
	if [ ${chr} == "1" ]; then echo "[3/8] Deducing A2 allele from REF, ALT, and A1."; fi
	mawk '$5==$7 { print $0, $6; next } 
		  $6==$7 { print $0, $5; next }
		  ' OFS='\t' ${targetDir}/chr/chr${chr}.txt > ${targetDir}/chr/chr${chr}.tmp.txt
	  	  \mv ${targetDir}/chr/chr${chr}.tmp.txt ${targetDir}/chr/chr${chr}.txt

	# calculate Z and ETA2 (with sex, age, age2, PC1-4 serving as covariates)
	if [ ${chr} == "1" ]; then echo "[4/8] Calculating Z and ETA2."; fi
...
echo "[7/8] Re-ordering and selecting columns in accordance with UKB sumstats."
...
sleep 1; chmod 770 ${targetDir}/sumstats.txt.gz
echo $'\n'"--- Completed: Harmonize LIFE with UKB sumstats ---"
