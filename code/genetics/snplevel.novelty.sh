...
prevDiscoveries="$2" # prevDiscoveries="data/prevDiscoveries/Jonsson_2019_munged.txt,data/prevDiscoveries/Ning_2020_munged.txt,data/prevDiscoveries/Smith_2020_munged.txt,data/prevDiscoveries/Leonardsen_2023_munged.txt data/prevDiscoveries/Wen_2024_munged.txt"
targetDir="$3" # targetDir="results/combined"
chrFilehandler="$4" # chrFilehandler='data/genetics/chr${i}/imp_mri_qc_EURjoined/bed/chr${i}_mri_qc'
LDsample="$5" # LDsample="data/gap_gm/gap_gm.txt"
clumpkb="$6" # clumpkb=10000
clumpr2="$7" # clumpr2=0.1
...
IFS=',' read -r -a prevDiscoveries <<< "${prevDiscoveries}"
> "${targetDir}"/snplevel.novelty.prevDiscoveries.txt
for i in "${prevDiscoveries[@]}"; do
    cat "${i}" >> "${targetDir}"/snplevel.novelty.prevDiscoveries.txt
done
...
awk 'NR==FNR { snp[$1]; next } $3 in snp { print $3 }' OFS='\t' <(echo "$variants") "$(eval echo "${chrFilehandler}".pvar)" | sort | uniq -c | awk '$1 > 1 { print $2}' >> "${targetDir}"/snplevel.novelty.prevDiscoveries.multiallelic.txt
) &
done
wait)
...
awk -F'\t' 'NR==1 { next } { print "ownDiscoveries", $3, $6, $12 }' OFS='\t' "${ownDiscoveries}" > "${targetDir}"/snplevel.novelty.ownDiscoveries.txt
...
for i in {1..22} X XY Y MT; do (
	plink2 --pfile $(eval echo "${chrFilehandler}") \
...
(for i in {1..22} X Y XY MT; do (
awk 'NR==FNR { snp[$3]=$1"\t"$2; next } $1 in snp { print $1, snp[$1] }' OFS='\t' $(eval echo "${chrFilehandler}".pvar) <(echo "${variants}") >> "${targetDir}"/snplevel.novelty.variants_chr_pos.txt
) &
done
wait)
...
# 1) get chr:pos of variants
	...
	for i in {1..22} X Y XY MT; do (
	awk 'NR==FNR { snp[$3]=$1"\t"$2; next } $1 in snp { print $1, snp[$1] }' OFS='\t' $(eval echo "${chrFilehandler}".pvar) <(echo "${variants}") >> "${targetDir}"/snplevel.novelty.prevDiscoveries.multiallelic_chr_pos.txt
	) &
	done
	wait
...
# 3) add to locus if distance is smaller than 10 MB
	awk -F'\t' -v clumpkb=${clumpkb} 'BEGIN { locus=1 } 
				FNR == 1 { filenum++ }
				...
				filenum == 3 { print }
				filenum == 3 && $1 != locus { locus=$1; for (key in id) if (locus_count[key]==locus && lead_snp_kb[key] <= clumpkb) { print locus_count[key], 9999, study[key], trait[key], id[key], p[key], lead_snp[key], lead_snp_kb[key], lead_snp_rsq[key], lead_snp_alleles[key], chr[key], bp[key], lead_snp_chr[key], lead_snp_bp[key]; delete id[key]}}
