...
awk -F'\t' -v header="${header}" '
	...
	if [ ! -z "$missings" ]; then
		...
		nrows=$(cat "${targetDir}/annovar/synonyms.rownum.txt" | awk 'END { print NF } ')
 		if [[ $nrows != 0 ]]; then
...
awk -F'\t' -v cols="${chr_bp_ref_alt}" -v header="${header}" '
	...
	OFS='\t' "${outputFile}" <(cat ${targetDir}/annovar/multianno/output.annovar.hg19_multianno.txt | grep 'nonsynonymous\|frameshift\|stopgain\|stoploss') \
