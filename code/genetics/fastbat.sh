...
# get arguments
trait="$1" # trait="gap_gwm"
targetDir="$2" # targetDir="results/${trait}/fastbat"
...
pthreshMapping=$6 # pthreshMapping=5E-8
glist_hg19="$7" # glist_hg19="data/glist_hg19/glist-hg19-refseq.txt"
threads=$8 # threads=52
window=$9 # window=0
humandb="${10}" # humandb="data/annovar/humandb"
refseq="${11}" # refseq="data/refseq/GRCh37_latest_genomic.edit.gff.gz"
clumpingWindow=${12} # clumpingWindow=10000
...
awk '{ print $1, $2 }' OFS="\t" "data/${trait}/${trait}.txt" > "${targetDir}/fastbat.sample.txt"

# get SNPs and P-values from summary statistics
awk 'BEGIN { print "SNP\tP" } NR > 1 { print $3, $10 }' OFS="\t" <(gzip -dc "${sumstats}") > "${targetDir}/fastbat.assoc.txt"
