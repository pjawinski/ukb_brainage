...
inputFiles = args[2] # inputFiles="results/gap_gm/magma/magma.genes.out.annot.clumped,results/gap_wm/magma/magma.genes.out.annot.clumped,results/gap_gwm/magma/magma.genes.out.annot.clumped"
clumpingWindow = as.numeric(args[3]) * 1000 # clumpingWindow = 3000 * 1000
outputFile = args[4] # outputFile="results/combined/magma.txt"

message(paste0('\n--- Combine MAGMA results across traits ---',
               '\ntraits: ', traits,
...
for (i in 1:length(traits)) {
  ...
  if (i ==1) {
    tmp = tmp[,c('GENE','SYMBOL','GENE_DESCRIPTION','GENE_BIOTYPE','ENTREZ_ID','HGNC_ID','CYTOBAND','CHR','START','STOP','NSNPS','NPARAM','N',
               'ZSTAT','P','FDR','LOCUS_COUNT','GENE_COUNT')]
  } else { 
    tmp = tmp[,c('GENE','ZSTAT','P','FDR','LOCUS_COUNT','GENE_COUNT')]
  }
  
  # create unique variable names
  names(tmp)[which(names(tmp) %in% c('ZSTAT','P','FDR','LOCUS_COUNT','GENE_COUNT'))] = 
    paste0(traits[i],c('_ZSTAT','_P','_FDR','_LOCUS_COUNT','_GENE_COUNT'))
...
for (i in 1:length(traits)) {
  cols = c(cols,'NA',paste0(traits[i],c('_ZSTAT','_P','_FDR','_LOCUS_COUNT','_GENE_COUNT')))
}
