...
# merge datasets
df = left_join(prs,data,by = 'IID')

# run prs analysis
message('Running prs analysis.')
for (i in ncol(prs):3) {
  options(warn = -1)
  tmp = data.frame(pthresh = names(prs)[i], pcor.test(x = df[,names(prs)[i]], y = df[,trait], z = df[,covs], method = 'pearson'))
  tmp$R2 = tmp$estimate^2
  tmp$df = tmp$n-2-tmp$gp
  tmp = tmp[,c('pthresh','estimate','R2','statistic','df','p.value','n','gp','Method')]
