...
repl = read.delim(replicationFile, sep = '\t', header = T)
...
df$A1_FREQround = paste0(format(round(df$A1_FREQ,3), nsmall = 3))
df$BETASE = paste0(format(round(df$BETA,3), nsmall = 3), ' (', format(round(df$SE,2), nsmall = 2), ')')
...
main = dfmerge[!duplicated(df$LOCUS_COUNT),c('CYTOBAND','CHR','BPstr', 'IDtrunc','A1A2','A1_FREQround', 'BETASE','Pstr',
              'REGION','GeneDistance','prioritized','TraitsPerLocus','ReplMain','LITERATURE_SHORT')]

# create supplementum data.frame
dfmerge$`NA` = ""
suppl = dfmerge[,c('LOCUS_COUNT','TraitNamesSuppl','CYTOBAND','CHR','BP',
              'ID','TYPED','INFO','A1','A2','A1_FREQ',
              'BETA','SE','Z','P','ETA2','N',
              'REGION','NEAREST_GENE','NEAREST_GENE_DESCRIPTION','NEAREST_GENE_BIOTYPE','DISTANCE',
              'FINEMAP_regionalh2','FINEMAP_bestK','FINEMAP_credibleSet_size',
              'NA','FINEMAP_genes','nonsyn_customPthresh', 'SMR_eqtl','SMR_sqtl','GTEx_singleTissue','GTEx_multiTissue','PoPS','prioritized',
              'NA','GWAS_CATALOG','LITERATURE_LONG')]

# write results
message('Writing output file.')
write.table(main, file = outputFileMain, row.names = F, quote = F, sep = '\t', na = "NA")
write.table(suppl, file = outputFileSuppl, row.names = F, quote = F, sep = '\t', na = "NA")
